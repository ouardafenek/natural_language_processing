mùERISK 2018: EARLY RISK PREDICTION ON THE INTERNET: EXPERIMENTAL FOUNDATIONS
===========================================================================
...
The test collection for this task is a collection of writings (posts or comments) from a set of Social Media users. 
There are two categories of users, anorexia and non-anorexia, and, for each user, the collection contains a sequence of writings (in chronological order). For each user, his collection 
...
(we will provide all chunks of all training users), and we will indicate what users have explicitly mentioned that they have been diagnosed with anorexia. The participants can therefore 
...
to process the data and, before the next release, each participating system has to choose between two options: a) emitting a decision on the user (i.e. anorexia or non-anorexia), 
...
- risk_golden_truth.txt: this file contains the ground truth (one line per subject). The code 1 means that the subject is a risk case of anorexia, while 0 means that the subject is a non-risk case.
- positive_examples: this folder, which stores all the posts of the risk cases, contains 10 subfolders. Each subfolder corresponds with one chunk. Chunk 1 contains the oldest
...
- negative_examples: this folder, which stores all the posts of the non-risk cases, contains 10 subfolders. Each subfolder corresponds with one chunk. Chunk 1 contains the oldest
writings of all users (first 10% of submitted posts or comments), chunk 2 contains the second oldest writings, and so forth. The name of the files follows de convention: <subjectname>_<chunknumber>.xml .
...
NOTE: put exactly two tabs between the subject name and the CODE (aggregate_results.py assumes that the files have this format)
...
CODE is your decision about the subject: three possible values......  0 means that you don't want to emit a decision on this subject (you want to wait and see more evidence)
																	  1 means that you want to emit a decision on this subject, and your decision is that he/she is a risk case of anorexia
																	  2 means that you want to emit a decision on this subject, and your decision is that he/she is NOT a risk case of anorexia
...
This script creates a file, e.g. usc_global.txt, which stores your final decision on every subject and the number of writings that you saw before making the decision.
...
[Losada & Crestani 2016] David E. Losada, Fabio Crestani. A Test Collection for Research on anorexia and Language use. 7th Conference Labs of the Evaluation Forum, CLEF 2016, Évora (Portugal), 2016